...
USERNAME = "zorooo"
PASSWORD = "135799"
BOT_TOKEN = "8546154661:AAFXheSvu4zJG8xUq1QDApaO-K5WX-OWo7M"
CHAT_IDS = [
    "-1003350573043",
]
REFRESH_INTERVAL = 7
...
def send_welcome(message):
    if is_banned(message.from_user.id):
        bot.reply_to(message, "🚫 انت محظور من البوت.")
        return
    ...
    if is_admin(message.from_user.id):
        admin_btn = types.InlineKeyboardButton("🔐 لوحة الادمن", callback_data="admin_panel")
        markup.add(admin_btn)
    bot.send_message(message.chat.id, "🌍 اختر دولة  👇", reply_markup=markup)
...
def handle_country_selection(call):
    ...
    if not force_sub_check(call.from_user.id):
        markup = force_sub_markup()
        if markup:
            bot.send_message(call.message.chat.id, "🔒 يجب الاشتراك فبيَ قنوات البوت.", reply_markup=markup)
        else:
    ...
    name, flag, _ = COUNTRY_CODES.get(country_code, ("مجهول", "🌍", ""))
    msg_text = f"""📱 الرقم: <code>{assigned}</code>
🌍 دولة : {flag} {name}
⏳ في انتظار otp ..📱"""
    markup = types.InlineKeyboardMarkup()
    markup.add(types.InlineKeyboardButton("🔄 تغير الرقم", callback_data=f"change_num_{country_code}"))
    markup.add(types.InlineKeyboardButton("🔙 رجوع Country", callback_data="back_to_countries"))
...
⏳ في انتظار. otp.📱"""
    markup = types.InlineKeyboardMarkup()
    markup.add(types.InlineKeyboardButton("🔄 تغيير الرقم", callback_data=f"change_num_{country_code}"))
    markup.add(types.InlineKeyboardButton("🔙 رجوع Country", callback_data="back_to_countries"))
    bot.edit_message_text(msg_text, call.message.chat.id, call.message.message_id, reply_markup=markup, parse_mode="HTML")
...
def admin_main_menu():
    markup = types.InlineKeyboardMarkup()
    btns = [
        types.InlineKeyboardButton("📥 اضافة كومبو", callback_data="admin_add_combo"),
        ...
        types.InlineKeyboardButton("📢 بث الكل ", callback_data="admin_broadcast_all"),
        types.InlineKeyboardButton("📨 مستخدم البث ", callback_data="admin_broadcast_user"),
...
def admin_panel(call):
    if not is_admin(call.from_user.id):
        return
    bot.edit_message_text("🔐 لوحة الادمن", call.message.chat.id, call.message.message_id, reply_markup=admin_main_menu())
...
def confirm_del_combo(call):
    ...
    delete_combo(code)
    name, flag, _ = COUNTRY_CODES.get(code, ("مجهول", "🌍", ""))
    markup = types.InlineKeyboardMarkup()
    markup.add(types.InlineKeyboardButton("🔙 Back", callback_data="admin_panel"))
...
def send_otp_to_user_and_group(date_str, number, sms):
    ...
    if user_id:
        try:
            ...
            markup = types.InlineKeyboardMarkup()
            markup.row(
                types.InlineKeyboardButton("𝑫𝑬𝑽", url="https://t.me/fnffo"),
                types.InlineKeyboardButton("𝑪𝒉𝒂𝒏𝒏𝒆𝒍", url="https://t.me/ayhamotp")
            )
            bot.send_message(user_id, f"رمز OTP الخاص بك  , ~ من {service}:\n🔑 <code>{otp_code}</code>", reply_markup=markup, parse_mode="HTML")
        except Exception as e:
...
def send_to_telegram_group(text):
    ...
    keyboard = {
        "inline_keyboard": [
            [
                {"text": "𝑫𝑬𝑽", "url": "https://t.me/FNFFO"},
                {"text": "𝑪𝒉𝒂𝒏𝒏𝒆𝒍", "url": "https://t.me/AYHAMOTP"}
            ]
        ]
    }
    success_count = 0
...
<blockquote>📩 <b>الرسالة الكاملة:</b>
